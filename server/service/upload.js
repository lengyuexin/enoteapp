...
const _commonCode = (file, ctx) => {
    ...
    const { proName, pageName } = ctx.request.body;
    if (proName == null || pageName == null) {
        error(ctx, 500, "上传失败：参数错误");
        return;
    }
    ...
    let filePath = path.resolve(__dirname, `${getImgTempZonePath(proName)}/${file.name}`);

    let onlyName = `${getFileName(filePath, 3)}--(未保存)${getFileName(filePath, 2)}`;

    // 创建可写流

    let target = path.resolve(__dirname, `${getImgTempZonePath(proName)}/${pageName}`);
    ...
    const upStream = fs.createWriteStream(`${target}/${onlyName}`);
